...
import materials from './materials.json'; // Import local materials

interface RadiusProBuilderFormProps {
...
export function RadiusProBuilderForm({ 
  product, 
  initialConfig, 
  onConfigChange, 
  onFieldFocusChange,
  ...
  useEffect(() => {
    ...
    if (needsMaterials) {
      try {
        console.log('[RadiusProBuilderForm] Loading local materials:', materials.length, 'materials');
        ...
        console.error("Error loading materials:", message);
    ...
    switch (param.type) {
      ...
      case 'select': {
        ...
        if (param.optionsSource === 'materials') {
          ...
          } else if (radiusProMaterials) {
            options = radiusProMaterials.map(mat => ({ value: mat.id, label: mat.name }));
          } else {
      ...
      {materialParam && (
          <div>
              ...
              <Select
                ...
                disabled={materialsLoading || !!materialsError || !radiusProMaterials}
                className="w-full"
              >
                <option value="" disabled>
                  {materialsLoading ? 'Loading...' : materialsError ? 'Error' : 'Select Material...'}
                </option>
                {radiusProMaterials?.map(mat => (
